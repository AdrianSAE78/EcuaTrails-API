...
public class TransportService {
	...
	public TransportService(InterestPointTransportRepository iptRepository, InterestPointRepository ipRepository, RouteRepository routeRepository) {
	...
	public RouteTransport getByRoute(Integer routeId) {
		...
		var items = iptRepository.findActiveByInterestPointIds(poiIds).stream()
				.map(Mappers::toTransportDto).collect(Collectors.toList());
		return new RouteTransport(routeId, items);
	}
	...
	public Directions directions(double originLat, double originLng, double destLat, double destLng, String mode) {
		...
		// average speed (approximate): walking 4.5 km/h, driving 40 km/h, transit 25 km/h
